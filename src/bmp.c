...
bool bmp_pwr_config(const bmp_data* bmp, bool pressure_enable, bool temperature_enable, uint8_t mode) {
  uint8_t value = ((uint8_t)pressure_enable) | (((uint8_t)temperature_enable) << 1) |
    (mode << 4);
...
float bmp_read_pressure(const bmp_data* bmp, float temperature) {
  float comp_press, partial_data1, partial_data2, partial_data3, partial_data4, partial_out1, partial_out2;
  ...
  partial_out2 = (float)press_raw * (bmp->calibration.p1 + partial_data1 + partial_data2 + partial_data3);

  partial_data1 = (float)press_raw * (float)press_raw;
  partial_data2 = bmp->calibration.p9 + bmp->calibration.p10 * temperature;
  partial_data3 = partial_data1 * partial_data2;
  partial_data4 = partial_data3 + ((float)press_raw * (float)press_raw * (float)press_raw) * bmp->calibration.p11;
